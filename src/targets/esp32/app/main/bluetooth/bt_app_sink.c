...
extern void local_write_audio(const uint8_t *data, uint32_t len);
extern void local_set_volume(uint8_t volume);
extern void local_active_audio(bool active);
extern void local_metadata_updated(char* artist, char* album, char* title, int duration);
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param) {
    ...
    case ESP_A2D_CONNECTION_STATE_EVT: {
        ...
        if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_DISCONNECTED) {
            // set_led_mode(BLT_SEARCHING);

            // nvs_blt_addr(1, 2, 0); // Reset stored bda.

            esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE,
                                     ESP_BT_GENERAL_DISCOVERABLE);
        } else if (a2d->conn_stat.state == ESP_A2D_CONNECTION_STATE_CONNECTED) {
            uint64_t integer;
            uint8_t blt_addr[8]; // Pack bda into a uint64.
            blt_addr[0] = bda[0];
            blt_addr[1] = bda[1];
            blt_addr[2] = bda[2];
            blt_addr[3] = bda[3];
            ...
            blt_addr[6] = 0;
            blt_addr[7] = 0;
            memcpy(&integer, &blt_addr, 8);
            // nvs_blt_addr(integer, 1);

            for (int i = 0; i < 8; i++) {
                if (i == 0) {
                    printf("\n");
                }
                printf("%02x:", blt_addr[i]); // Print address
                if (i == 7) {
                    printf("\n");
                }
            }
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param) {
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        ...
        local_metadata_updated(s_metadata.artist, s_metadata.album, s_metadata.title, s_metadata.duration);
...
void volume_set_by_controller(uint8_t volume) {
    uint8_t volumePercent = volume * 100 / 0x7f;
    ESP_LOGI(BT_RC_TG_TAG, "Volume is set by remote controller %d%%\n",
             (uint32_t)volume * 100 / 0x7f);

    local_set_volume(volumePercent);
