...
void i2cMasterWriteToDevice(int portNum, uint8_t deviceAddr,
                            berry::list writeBuf) {
...
berry::list i2cMasterWriteReadDevice(int portNum, uint8_t deviceAddr,
                                     berry::list writeBuf, int readSize) {
  auto writeBufMapped = std::vector<uint8_t>();
  for (auto const& value : writeBuf) {
    writeBufMapped.push_back(std::any_cast<int>(value));
  }
...
void exportI2CDriver(std::shared_ptr<berry::VmState> berry) {
  ...
  berry->export_function("master_write_to_device", &i2cMasterWriteToDevice,
                         "i2c");
  berry->export_function("master_read_from_device", &i2cMasterReadFromDevice,
                         "i2c");
  berry->export_function("master_write_read_device", &i2cMasterWriteReadDevice,
                         "i2c");
  berry->export_function("is_installed", &i2cIsInstalled, "i2c");
}
