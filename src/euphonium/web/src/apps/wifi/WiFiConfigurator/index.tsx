...
import { WiFiNetwork, WiFiState } from "../../../api/euphonium/wifi/models";
...
const ConnectedState = ({ ipAddr = "" }) => {
  return (
    <div class="flex flex-col space-y-3">
      <div class="text-gray-400 text-s flex-row flex">
        WiFi connected successfully
      </div>
      <div class="text-sm text-gray-400 pb-1">
        your address is <i class="text-green-400">{ipAddr}</i>. Please connect
        to your network and connect via this address.
      </div>
      <Button
        onClick={() => {
...
const NetworkScanResults = ({
  ...
  return (
    <div class="flex flex-col space-y-3">
      <p>
        <div class="text-gray-400 text-s mb-1 flex-row flex">
          available networks (
          <div class="ml-1 mr-1 text-green-400 text-s animate-pulse">
            scanning
          </div>
          )
        </div>
...
export default () => {
  ...
  const handleOnOnline = useCallback(() => {
    wifiAPI.getWifiStatus().then(setWifiState);
  }, []);
  ...
  const startConnectWifi = async (ssid: string, password: string) => {
    wifiAPI.connectToWifi(selectedNetwork!!.ssid, password);
  };
  ...
  return (
    <div class="md:w-[600px] md:absolute md:left-1/2 md:transform md:-translate-x-1/2 w-full">
      ...
      >
        {!selectedNetwork && wifiState.state == "scanning" ? (
          <NetworkScanResults
            wifiState={wifiState}
        ...
        {wifiState.state == "connected" ? (
          <ConnectedState ipAddr={wifiState.ipAddress} />
        ) : null}
      </Card>
    </div>
  );
