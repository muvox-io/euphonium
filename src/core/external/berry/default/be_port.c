...
static FILE *stdout_definition = NULL;
static FILE *stdin_definition = NULL;

BERRY_API void be_set_stdout(void *redefined_stdout) {
  stdout_definition = redefined_stdout;
}

BERRY_API void be_set_stdin(void *redefined_stdin) { stdin_definition = redefined_stdin; }

BERRY_API void be_writebuffer(const char *buffer, size_t length) {
  if (stdout_definition == NULL) {
    stdout_definition = stdout;
  }

  be_fwrite(stdout_definition, buffer, length);
}

BERRY_API char *be_readstring(char *buffer, size_t size) {
  if (stdin_definition == NULL) {
    stdin_definition = stdin;
  }

  return be_fgets(stdin_definition, buffer, (int)size);
}
...
long int be_ftell(void *hfile) { return f_tell(hfile); }

long int be_fflush(void *hfile) { return f_sync(hfile); }

size_t be_fsize(void *hfile) { return f_size(hfile); }
...
int be_chdir(const char *path) { return f_chdir(path); }

int be_mkdir(const char *path) { return f_mkdir(path); }

int be_unlink(const char *filename) { return f_unlink(filename); }
...
char *be_getcwd(char *buf, size_t size) { return _getcwd(buf, (int)size); }

int be_chdir(const char *path) { return _chdir(path); }

int be_mkdir(const char *path) { return _mkdir(path); }
...
int be_dirclose(bdirinfo *info) { return closedir(info->dir) != 0; }
