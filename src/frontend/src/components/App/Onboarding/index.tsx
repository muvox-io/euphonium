...
import { ConfigurationField, PluginConfiguration } from "../../../api/euphonium/plugins/models"
...
const HardwareDetectedModal = ({ configUpdated }: any) => {
  ...
  const confirmSetting = async () => {
    await pluginsAPI.updatePluginConfiguration("general_settings", {
      "onboardingHardware": "true"
    }, false);
    configUpdated();
  }

  return (<Modal header="Detected official hardware">
    ...
    <Button onClick={confirmSetting} class="primary text-xl h-[50px]">Confirm</Button>
  </Modal>)
}
...
const VolTuningButton = ({ isNegative, onTap, disabled }: any) => {
  return (
    <button onClick={() => {
      if (disabled) return;
      onTap();
    }} class={`w-[50px] h-[50px] rounded-xl flex flex-col items-center pt-[6px] text-[26px]` + (!disabled ? ' hover:bg-app-secondary-dark bg-app-secondary' : '')}>
      {isNegative ? '-' : '+'}
    </button>
  );
}
...
const VolumeTuningModal = ({ configUpdated }: any) => {
  ...
  useEffect(() => {
    if ( !playbackState?.settings || playbackState.settings?.state || playbackState.settings.state == "queue_empty") {
      // TODO: replace with fs tune
      playbackAPI.updateVolume(0);
      playbackAPI.queryContextURI("radio://https%3A%2F%2Fs2.radio.co%2Fs2b2b68744%2Flisten")
    }
  }, []);
  ...
  return (<Modal header="Amplifier loudness tuning">
    <div class="text-[16px] text-app-text-secondary mt-4 mb-10 lg:w-[420px] leading-6">
      Keep increasing the volume until the tune being played is at the preferred maximum volume level. In case the sound becomes disorted, decrease it.
    </div>

    <div class="flex flex-row items-center mb-10 w-full">
      <VolTuningButton disabled={volume == 0} onTap={decreaseVolume} isNegative={true} />
      <div class="flex-grow flex flex-col justify-center items-center">
        <div class="text-[16px] text-app-text-secondary">Volume</div>
        <div class="text-[32px] text-app-text-primary">{volume}%</div>
      </div>
      <VolTuningButton disabled={volume == 100} onTap={increaseVolume} isNegative={false} />
    </div>
    <Button onClick={confirmMaxVolume} class="primary text-xl h-[50px]">Confirm</Button>
...
export default ({ }) => {
  ...
  const loadConfig = async () => {
    const { configSchema } = await pluginsApi.getPluginConfiguration("general_settings");
  ...
  for (const onboardingStepKey of Object.keys(onboardingSteps)) {
    const onboardingStepConfig = schema.find((field) => field.key == onboardingStepKey);
    if (onboardingStepConfig && (onboardingStepConfig.value == "false" || !onboardingStepConfig.value)) {
