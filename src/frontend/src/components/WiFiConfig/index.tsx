...
const FloatingCard = ({ children, header, className }: any) => {
  return (
    <div class="bg-app-primary w-full lg:w-auto h-full lg:h-auto text-3xl text-app-text-primary pr-8 pt-8 pb-6 pl-8 lg:rounded-[40px] m-auto flex flex-col">
      {header}
      {children}
    </div>
  );
...
const renderHello = () => {
  return (
    <FloatingCard header="Hello!">
      <div class="text-[16px] text-app-text-secondary mt-4 mb-20 w-[500px] leading-6">
        Thanks for trying out Euphonium. This wizard will guide you through the
        initial setup of your device.
        <br />
        <br />
        Before we begin, make sure you have access to a stable network, and know
        it’s credentials.
      </div>
...
const NetworkItem = ({ name, isOpen, onClick }: any) => {
  return (
    <div
      onClick={onClick}
      class="flex flex-row items-center text-lg pr-2 pl-6 h-[55px] text-app-text-primary rounded-2xl mt-2 mb-2 bg-app-secondary hover:opacity-70"
    >
      {name}
      {!isOpen ? (
        <div class="ml-auto">
...
const NetworkList = ({ setConfigState, connectivity }: NetworkListParams) => {
  ...
  return (
    <FloatingCard header="Configure WiFi">
      <div class="text-[16px] text-app-text-secondary mt-3 mb-2 w-full lg:w-[500px] leading-6 flex flex-col">
        ...
        {hasNetworks ? (
          connectivity?.body?.networks?.map((network: any) => {
            return (
              <NetworkItem
                key={network.ssid}
                name={network.ssid}
      ...
      <div class="text-[16px] text-app-text-secondary mt-3 mb-2 w-auto lg:w-[500px] leading-6">
        ...
        {network?.open ? null : (
          <>
            ...
            <Input
              type="password"
              class="-mt-2 lg:mt-2 mb-2 text-2xl"
              height={55}
        ...
        <div
          class={`${
            connectivity?.body?.error ? "visible" : "invisible"
          } text-red-500 text-l mb-4`}
        >
          Connection failed, recheck credentials
        </div>
        <Button
          disabled={!network?.open && password == ""}
          onClick={() => connect()}
        ...
        <div
          ...
          class="mt-3 w-full text-center cursor-pointer"
        >
          Go back
        </div>
      </div>
  ...
  return (
    <FloatingCard header="Configure WiFi">
      <div class="text-[16px] text-app-text-secondary mt-3 mb-2 w-auto lg:w-[500px] leading-6">
        ...
        <div class="mt-4 mb-4 text-sm text-green-300">
          Note: Your browser will now connect back into your previous network,
          and redirect to the device
        </div>
        <Button
          onClick={() => {}}
          class="primary text-xl h-[55px] w-full flex flex-row justify-center items-center"
