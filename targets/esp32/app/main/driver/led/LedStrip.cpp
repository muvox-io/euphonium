...
IsrCore LedStrip::_interruptCore = CoreCurrent;
intr_handle_t LedStrip::_interruptHandle = NULL;

LedStrip *&IRAM_ATTR LedStrip::ledForChannel(int channel) {
    static LedStrip *table[8] = {nullptr};
    assert(channel < 8);
    return table[channel];
}

void IRAM_ATTR LedStrip::interruptHandler(void *) {
...
void IRAM_ATTR LedStrip::copyRmtHalfBlock() {
    ...
    for (i = 0; i != len && _pixelPosition != _count; i++) {
        ...
        val = scale8_video(val, _brightness);
