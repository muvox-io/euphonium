...
export default function () {
  ...
  const loadFile = async (file: any) => {
    const response = await fetch("http://192.168.1.234/file" + file);
  ...
  const loadLogs = async () => {
    const response = await fetch("http://192.168.1.234/request_logs");
    const json = await response.text();
    setLogs(json.split("\n"));
    setTimeout(() => {
  ...
  const updateFile = async (selectedFile: any) => {
    const response = await fetch("http://192.168.1.234/file" + selectedFile, {
      method: "POST",
      body: fileContent,
    });
    await response.text();
    ...
    <div class="flex overflow-auto h-screen w-screen flex-col bg-app-secondary">
      <div class="flex flex-row">
        <div class="text-white text-xl m p-3 font-bold flex bg-app-primary h-screen flex-col w-[220px] h-screen">
          ...
          <FileTree fileSelected={(file) => loadFile(file)} />
